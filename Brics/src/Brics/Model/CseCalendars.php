...
class CseCalendars extends DbModel implements DbModelFactoryInterface {
	protected $primary_key = 'id';
	
	public function __construct(Adapter $dbh, $database = 'brics') {
		parent::__construct($dbh, new TableIdentifier('csecalendar', $database));
	}
	...
	public function filter($filter, $range, $sort, &$total) {
		...
		$result = $this->getGateway()->select(function($select) use($self, $filter, $range, $sort, $table, $database) {
			$select->join(array('U' => new TableIdentifier('users', $database)),
						  "{$table}.cse = U.id",
						  array(
								'username'
						  ),
