...
use Hr\Model\HrGroups as HrHrGroups;
use Zend\ServiceManager\ServiceLocatorInterface;

class HrGroups extends HrHrGroups {
	...
	public function filterAvailable($filter, $range, $sort, &$total) {
		...
		$result = $this->getGateway()->select(function($select) use($self, $filter, $range, $sort, $table, $database, $auth_database) {
			$select->columns(array(
				'id',
				'name',
				'unavailable' => new Expression("MAX(IF(GRL.role_id = ?, 1, 0))", array($filter['role_id'])),
			));
			...
			$select->join(array('GRL' => new TableIdentifier('security_group_role_links', $auth_database)),
						  "{$table}.id = GRL.group_id",
						  array(),
						  Select::JOIN_LEFT);
